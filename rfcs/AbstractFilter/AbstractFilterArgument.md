...
type Hamster implements Animal & Pet {
  name: String!
  weight: Int!
}

type Sheep implements Animal {
  weight: Int!
}
```

It is possible to fetch types which 
...
Whereas the below example must fail, since `Sheep` does not implement the `Pet`
